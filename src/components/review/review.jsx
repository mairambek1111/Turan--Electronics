import {FaStar} from "react-icons/fa";
import './review.scss'

const Review = () => {
    return (
        <div id='review'>
            <div className="container">
                <div className="review">
                    <button><svg width="31" height="29" viewBox="0 0 31 29" fill="none" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                        <rect width="31" height="29" fill="url(#pattern0)"/>
                        <defs>
                            <pattern id="pattern0" patternContentUnits="objectBoundingBox" width="1" height="1">
                                <use xlinkHref="#image0_843_1856" transform="matrix(0.0103943 0 0 0.0111111 0.0322581 0)"/>
                            </pattern>
                            <image id="image0_843_1856" width="90" height="90" xlinkHref="data:image/png;base64,"/>
                        </defs>
                    </svg>
                        Написать отзыв
                    </button>
                    <div className="review--card">
                        <div className="review--card__about">
                            <div className="review--card__about--title">
                                <h1>Айгерим Атамбекова</h1>
                                <div className="review--card__about--title__stars">
                                    <FaStar className='starsYellow'/>
                                    <FaStar className='starsYellow'/>
                                    <FaStar className='starsYellow'/>
                                    <FaStar className='starsYellow'/>
                                    <FaStar className='starsNone'/>
                                </div>
                            </div>
                            <div className="review--card__about--title__date">
                                <span>02.02.2024</span>
                            </div>
                        </div>
                        <p>Заказала долгожданный свой Iphone на этом сайте, спасибо большое у вас цены всегда ниже,
                            чем в других местах. Заказ пришел вовремя, Успехов Вам всегда!</p>
                    </div>
                </div>
            </div>
        </div>
    );
};

export default Review;